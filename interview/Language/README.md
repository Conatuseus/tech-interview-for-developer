...
>
> - 인터프리터 방식은 적재된 바이트 코드를 실행시점에 한줄씩 기계어로 변환하는 방식입니다.
>   그러므로 전체적인 실행 속도가 느립니다.
> - JIT 컴파일러를 이용하는 방식은, 특정 시점에 바이트 코드 전체를 기계어로 변환해두고 실행하는 방식입니다.
>
...
>
> 그 다음 Remark 단계에서는 Concurrent Mark 단계에서 새로 추가되거나 참조가 끊긴 객체를 확인합니다. Remark 단계에서도 짧은 Stop-The-World가 발생합니다. 마지막으로 Concurrent Sweep 단계에서는 쓰레기를 정리하는 작업을 실행합니다. 이 작업도 다른 스레드가 실행되고 있는 상황에서 진행합니다.
>
...
>
> 참조 타입은 기본형 타입을 제외한 타입으로 클래스 타입, 인터페이스 타입, 배열 타입이 있습니다. 빈 객체를 의미하는 Null이 허용됩니다. 그리고 값이 저장되어 있는 곳의 주소값을 저장하는 공간으로 힙(Heap) 메모리에 저장됩니다.
